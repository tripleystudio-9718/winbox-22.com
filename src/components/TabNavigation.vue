...
// Import tab icons
import top10Icon from "@/assets/top10-icon.png";
import top10ActiveIcon from "@/assets/top10-active-icon.png"
...
import horseIcon from "@/assets/horsing-icon.png";
import horseActiveIcon from "@/assets/horsing-active-icon.png";
...
const tabs = [
  {
    ...
    name: "TOP 10",
    icon: top10Icon, 
    activeIcon: top10ActiveIcon, 
    route: "/", 
    component: defineAsyncComponent(() =>
      import("@/components/tabs/Top10Tab.vue")
    ),
  },
  {
    id: "slots",
  ...
  },
  {
    ...
    route: "/horse-racing-rcb988",
    component: defineAsyncComponent(() =>
      import("@/components/tabs/HorsingTab.vue")
    ),
  },
];

// Active tab state - initialize based on current route
const getActiveTabFromRoute = () => {
  const currentPath = route.path;
  const tab = tabs.find(t => t.route === currentPath);
  return tab ? tab.id : "top10"; // Default to top10 if no match
};

const activeTab = ref(getActiveTabFromRoute());

// Handle tab click - set active tab and update URL
const handleTabClick = (tab) => {
  setActiveTab(tab.id);
  // Update URL without page reload, but stay in tab navigation
...
.desktop-tab-header .tab-item {
  width: 16.66%; /* Adjusted for 6 tabs */
  padding: 8px;
  flex-shrink: 0;
}
